<template>
  <div>
    ...
    <mu-list>
      <mu-list-item v-for="courseMessage in list" :value="courseMessage.id" :title="courseMessage.title" :describeText="getString(courseMessage.name, courseMessage.time, courseMessage.reply)" :afterText="getState(courseMessage.state)" @click="goEdit(courseMessage.id)">
        <mu-icon v-if="courseMessage.state.toString() === '0'" slot="left" color="#9e9e9e" value="sentiment_very_dissatisfied" :size="40" />
        <mu-icon v-if="courseMessage.state.toString() === '1'" slot="left" color="#8bc34a" value="sentiment_neutral" :size="40" />
      </mu-list-item>
    </mu-list>
    <mu-flexbox>
