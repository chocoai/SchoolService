...
public class CourseRoomTeacherSemester_Save implements Interceptor {
  public void intercept(Invocation inv) {
    if (!StrKit.isBlank(inv.getController().getPara("cid"))
      ...
      if (Db.find("SELECT * FROM courseroomteachersemester WHERE course_id = ? AND room_id = ? AND semester_id = ? AND teacher_id = ?", cid,rid,sid,tid).size() != 0) {
