<template>
  <div class="StudentList">
    ...
    <mu-drawer right :open="openTeam" docked="false">
      <mu-appbar title="请选择社团" @click.native="closeTeam">
        <mu-icon-button icon='done' slot="right"/>
      </mu-appbar>
      <mu-list>
        <mu-list-item v-for="team in team" :title="team.name">
          <mu-icon v-if="team.state.toString() === '1'" slot="left" :size="40" value="store" color="#9c27b0"/>
          <mu-icon v-if="team.state.toString() === '2'" slot="left" :size="40" value="store" color="#ce93d8"/>
          <mu-radio v-model="team_id" label="" labelLeft :nativeValue="team.id" slot="right" iconClass="color: #215E21"/>
        </mu-list-item>
      </mu-list>
    </mu-drawer>
