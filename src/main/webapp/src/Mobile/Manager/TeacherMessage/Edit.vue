<template>
  <div>
    ...
    <mu-text-field label="内容" disabled v-model="content" fullWidth labelFloat icon="note" multiLine :rows="9" :rowsMax="12" :maxLength="300"/><br/>
  ...
  methods: {
    ...
    fetchData (id) {
      this.$http.get(
        ...
        this.state = this.notice.state
        this.send = this.notice.send
        this.name = response.body.teacher
