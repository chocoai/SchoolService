<template>
  <div class="TeamList">
    <mu-appbar title="">
      ...
      <mu-text-field icon="search" class="appbar-search-field" hintText="请输入社团名称" @input="query" :value="queryString"/>
...
<script>
  ...
  export default {
    ...
    created: function () {
      ...
      this.teamQuery(this.queryString, this.pageCurrent, this.pageSize)
      this.teamTotal(this.queryString, this.pageSize)
    },
    methods: {
      openMenu () {
        this.open = true
      ...
      teamQuery (queryString, pageCurrent, pageSize) {
        this.$http.get(
          API.QueryByName,
          { params:
          {
      ...
      teamTotal (queryString, pageSize) {
        this.$http.get(
          API.TotalByName,
          { params:
          {
      ...
      look (teamId) {
        this.$router.push({ path: '/teamEdit/' + teamId })
        this.$store.commit('save', {
          queryString: this.queryString,
          pageCurrent: this.pageCurrent
        })
