...
CREATE TABLE `TeacherMessageSend` (
...
`del` int NULL COMMENT '删除状态0可用1删除',
PRIMARY KEY (`id`) 
);
...
CREATE TABLE `ParentMessageSend` (
`id` int NOT NULL AUTO_INCREMENT COMMENT '序号',
`title` varchar(255) CHARACTER SET utf8 NULL COMMENT '消息标题',
`content` varchar(255) CHARACTER SET utf8 NULL COMMENT '消息内容',
`teacher_id` int NULL COMMENT '教师序号',
...
CREATE TABLE `ParentMessageRead` (
`id` int NOT NULL AUTO_INCREMENT,
`message_id` int NULL COMMENT '消息序号',
`parent_id` int NULL COMMENT '教师序号',
`time` datetime NULL COMMENT '阅读时间',
`state` int NULL COMMENT '是否阅读0未读1已读',
PRIMARY KEY (`id`) 
);
...
ALTER TABLE `TeacherMessageSend` ADD CONSTRAINT `teacherMessage_teacher` FOREIGN KEY (`teacher_id`) REFERENCES `Teacher` (`id`);
ALTER TABLE `TeacherMessageRead` ADD CONSTRAINT `teacherMessageRead_message` FOREIGN KEY (`message_id`) REFERENCES `TeacherMessageSend` (`id`);
...
ALTER TABLE `ParentMessageSend` ADD CONSTRAINT `parentMessage_teacher` FOREIGN KEY (`teacher_id`) REFERENCES `Teacher` (`id`);
ALTER TABLE `ParentMessageRead` ADD CONSTRAINT `parentMessageRead_message` FOREIGN KEY (`message_id`) REFERENCES `ParentMessageSend` (`id`);
ALTER TABLE `ParentMessageRead` ADD CONSTRAINT `parentMessageRead_parent` FOREIGN KEY (`parent_id`) REFERENCES `Parent` (`id`);
