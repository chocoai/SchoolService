...
`url` varchar(255) CHARACTER SET utf8 NOT NULL COMMENT '访问路径',
...
`teacher_id` int NOT NULL COMMENT '教师序号',
`permission_url` varchar(255) CHARACTER SET utf8 NOT NULL COMMENT '访问路径',
`state` int NULL COMMENT '权限状态0无1有',
PRIMARY KEY (`teacher_id`, `permission_url`) 
);
...
ALTER TABLE `TeacherPermission` ADD CONSTRAINT `teacherPermission_permission` FOREIGN KEY (`permission_url`) REFERENCES `Permission` (`url`);
